...
public class Spaceship {
	...
	private void updateCentreGravite() {
		masseTotale = 0;
		for (int i = 0; i < nbModules; i++) {
			masseTotale += modules[i].masse;
		}
		centreGravite.set(0, 0, 0);
		for (int i = 0; i < nbModules; i++) {
			centreGravite.x += modules[i].position.x * modules[i].masse;
			centreGravite.y += modules[i].position.y * modules[i].masse;
			centreGravite.z += modules[i].position.z * modules[i].masse;
		}
